...
from modules.pages import page_recomendacion as pr
...
def _logo_block_html() -> str:
    ...
    return (
        ...
        'letter-spacing:-0.01em;line-height:1.15;">AgroPlanificación</div>'
        '<div style="font-size:0.66rem;font-weight:700;color:#1565c0;'
        'letter-spacing:0.10em;text-transform:uppercase;margin-top:3px;">'
        'Tablero Ganadero</div>'
        '</div></div>'
    )
...
def _executive_summary_html() -> str:
    """Resumen ejecutivo: estrategia recomendada + 5 KPIs principales.

    Usa la lógica de page_recomendacion (ponderaciones + scores) para
    determinar la estrategia ganadora; luego saca de allí las 5 métricas
    a destacar."""
    pr._ensure_slider_state()
    data = pr._gather_stage_data()
    dim_scores = pr._compute_dim_scores(data)
    weights = pr._get_weights()
    strat_scores = pr._compute_strategic_scores(dim_scores, weights)
    top_key, runner_up = pr._winner(strat_scores)
    top = data[top_key]
    ...
    if runner_up is not None:
        ...
        title_html = (
            f'<span style="color:{color};">{icon} {title}</span>'
            f' <span style="color:#94a3b8;font-weight:600;font-size:0.85em;">+</span> '
            f'<span style="color:{runner["meta"]["color"]};">'
            f'{runner["meta"]["icon"]} {runner["meta"]["title"]}</span>'
        )
    else:
        title_html = f'<span style="color:{color};">{icon} {title}</span>'
    ...
    hero = (
        ...
        f'<div style="font-size:0.60rem;color:#94a3b8;font-weight:700;'
        f'text-transform:uppercase;letter-spacing:0.06em;">Score</div>'
        f'<div style="font-size:1.85rem;font-weight:800;color:{color};'
        f'line-height:1;letter-spacing:-0.02em;">{score:.0f}'
        f'<span style="font-size:0.84rem;color:#94a3b8;font-weight:600;">'
        f' / 100</span></div>'
        f'</div></div>'
    )
    ...
    margen_sign = "+" if margen >= 0 else "−"

    cards = (
        _kpi_card_html(
            "💵", "Margen bruto",
            f"{margen_sign}USD {abs(margen):,.0f}",
            margen_color, "USD/cab",
        )
        + _kpi_card_html(
            "⚡", "USD / cab / día",
            f"USD {top['usd_cab_dia']:.2f}",
        )
        + _kpi_card_html(
            "📈", "ROI operativo",
            f"{top['roi']:+.1f}%",
        ...
        + _kpi_card_html(
            "⚠", "Riesgo",
            f"{top['riesgo']:.0f}/100",
            _risk_color(top["riesgo"]),
        )
        + _kpi_card_html(
            "🛡", "Robustez",
            f"{top['robustez']:.0f}/100",
            _robust_color(top["robustez"]),
        )
    )
...
def _modelo_productivo_mini_html() -> str:
    ...
    t_e = t_c + ms["eng_exp"]["dias"]

    kg_birth = ms["cria"]["kg_in"]
    kg_final = ms["eng_exp"]["kg_out"]
...
def _ingresos_mini_html() -> str:
    ...
    ys = [
        ...
        ing["eng_exp"]["kg_out"] * ing["eng_exp"]["precio_venta"],
    ]
...
_STAGES_KEYS = ["cria", "recria", "eng_int", "eng_exp"]
...
def _sensibilidad_mini_html() -> str:
    ...
    weights = pr._get_weights()
    strat_scores = pr._compute_strategic_scores(dim_scores, weights)
    top_key, _runner = pr._winner(strat_scores)

    sens = ps._build_sensibilidad()
    s_top = sens[top_key]

    # Tornado para la etapa ganadora, métrica margen_cab
    rows, _baseline = ps._tornado_data(s_top, "margen_cab")
    # rows está orden ascendente por swing; tomar el de mayor swing
    if rows:
    ...
    robustez_top = data[top_key]["robustez"]
    riesgo_top = data[top_key]["riesgo"]
    ...
    # Chart: top swings del tornado (4 más altos)
...
# ── Mini-bloque: Recomendación Estratégica ──────────────────────────────────

def _recomendacion_mini_html() -> str:
    pr._ensure_slider_state()
    data = pr._gather_stage_data()
    dim_scores = pr._compute_dim_scores(data)
    weights = pr._get_weights()
    strat_scores = pr._compute_strategic_scores(dim_scores, weights)
    top_key, runner_up = pr._winner(strat_scores)
    sw = pr._strengths_weaknesses(dim_scores)

    top_score = strat_scores[top_key]
    top_meta = data[top_key]["meta"]

    fort_list = sw[top_key]["fortalezas"]
    debi_list = sw[top_key]["debilidades"]
    if fort_list:
        _, f_label, f_icon = fort_list[0]
        fort_text = f"Fortaleza clave: <b>{f_icon} {f_label}</b>"
    else:
        fort_text = "Fortaleza clave: <b>—</b>"
    if debi_list:
        _, d_label, d_icon = debi_list[0]
        debi_text = f"Riesgo a vigilar: <b>{d_icon} {d_label}</b>"
    else:
        debi_text = "Riesgo a vigilar: <b>—</b>"

    runner_text = ""
    if runner_up is not None:
        r_meta = data[runner_up]["meta"]
        runner_text = (f" · Alterna: <b>{r_meta['icon']} {r_meta['title']}</b>"
                       f" ({strat_scores[runner_up]:.0f})")

    bullets = [
        f"Ganador: <b>{top_meta['icon']} {top_meta['title']}</b> · "
        f"score <b>{top_score:.0f}/100</b>{runner_text}",
        fort_text,
        debi_text,
    ]

    # Chart: top 4 dimensiones del ganador (scores normalizados 0-100)
    items = []
    for d_key, d_label, d_icon, _dir in pr._DIMENSIONS:
        items.append((f"{d_icon} {d_label}", dim_scores[d_key][top_key]))
    items_top = sorted(items, key=lambda x: -x[1])[:4]
    chart = _hbars_html(items_top, color="#0c1a2e", top_n=4)

    return _mini_block_html("#0c1a2e", "🎯", "Recomendación",
                             bullets, chart)


# ── Tres columnas: segunda fila de mini resúmenes ───────────────────────────
